    ...
    var mp4 = function(opts, cb) {
        ...
        MP4Tag.parse = function(handle, callback) {
            ...
            function readString(ab, length, offset) {
                return ab2b(ab).toString("utf8", offset, length + offset)
            }
            var containers = ["moov", "udta", "mdia", "meta", "ilst", "stbl", "minf", "moof", "traf", "trak", "stsd"];
            var atom_data_types = {
                0: "IMPLICIT",
                1: "UTF8",
                2: "UTF16",
            ...
            var extractors = {
                ...
                readBool: function readBool(view, length) {
                    return view.byteLength ? view.buffer[0] != "\x00" : false
                },
            ...
            var field_names = ["title", "date", "artist", "album", "tracknumber", "comment"],
                ...
                if (!err) {
                    ...
                    readAllMetadata(ilst.children, atom_names, function(err, datum) {
                        if (!err) {
                            ...
                            tags.year = new Date(Date.parse(tags.date)).getUTCFullYear();
                            tags.track = tags.tracknumber.join("/");
                            callback(null, tags, datum)
                        } else {
                            callback(err)
                        }
                    })
        ...
        handle.open(options.file, function(err) {
            ...
            MP4Tag.parse(handle, function(err, tags, raw) {
                cb(err, tags, raw)
            })
        })
    };
